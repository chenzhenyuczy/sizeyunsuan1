...
void getNumber(int &a,int &b,int &c,int &d,int &p)
{
	
	a=rand()%p;//生成一个100以内的随机整数
	b=rand()%p;
	c=rand()%p;
	d=rand()%p;
}
...
void printfsuanshi(int &a,int &b,int &c,int &d,int &r,int &mode1,int &mode2,int &mode3,int *i,int&n)
{
	printf("%d",a);
	mode(b,mode1,*i);
	printf("%d",b);
...
{	
	...
	for(int i=0;i<n;i++)
	{
		...
		m=i+1;
		printf("%d: ",m);
		printfsuanshi(a,b,c,d,r,mode1,mode2,mode3,&i,n);
part2 :
		printf("");
	}
	return 0;
